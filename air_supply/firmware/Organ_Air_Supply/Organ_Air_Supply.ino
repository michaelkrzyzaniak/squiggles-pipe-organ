...
float    PID_TARGET_PRESSURE     =    2.25; //inches H2O
bool     pid_is_initalized = false;
float    pid_p=0.003, pid_i=0.003, pid_d=0.02;
float    pid_prev_error = 0;
float    pid_integral_error = 0;
uint32_t pid_prev_time = 0;
float    high_res_duty = 0;
...
float pid_update(float pressure, float target_pressure)
{
  ...
  uint32_t t  = millis();
  uint32_t dt = t - pid_prev_time; //correct even when millis overflows
  float    deriv = (dt > 0) ? ((e - pid_prev_error) / (float)dt) : 0;
  ...
  if(pid_integral_error > 100)
    pid_integral_error = 100.0 / pid_i;
  ...
  //Serial.print(" : ");
  //Serial.println(pid_p*e + pid_i*pid_integral_error + pid_d*deriv);
...
void setup() 
{
  ...
  fan_set_duty(50 , true);
  fan_set_duty(50, false);  
  high_res_duty = 50;
  delay(5000);
}
      ...
      switch(Serial.read())
        {
          ...
          */
          case 'p':
            pid_set_p(pid_p - 0.001);
            Serial.print("pid_p = ");
            Serial.println(pid_p*10);
            break;
          case 'P':
            pid_set_p(pid_p + 0.001);
            Serial.print("pid_p = ");
            ...
            break; 
          case 'i':
            pid_set_i(pid_i - 0.0001);
            Serial.print("pid_i = ");
            Serial.println(pid_i*100);
            break;
          case 'I':
            pid_set_i(pid_i + 0.0001);
            Serial.print("pid_i = ");
          ...
          case '=': /*cascade*/
          case '+':
            pid_set_target_pressure(PID_TARGET_PRESSURE + 0.05);
...
*/  
    ...
    fan_set_duty(high_res_duty, false);    
