...
Bode* bode_new_with_new_one_second_recording_private(int num_channels)
{
  ...
  //Synth* s = syNew(); //sin sweep
  Synth2* s = sy2New();  //unit sample
  if(s == NULL){perror("unable to create synth\r\n"); return NULL;}

  Microphone* m = micNew(num_channels, 2 /*secs*/);
  if(m == NULL){perror("unable to create microphone\r\n"); return NULL;}
  ...
  aiff = micGetRecording(m);
  
  //aiffTrimBeginning(aiff, 0.04, aiffYes);  
  //aiffTrimEnd      (aiff, 0.04, aiffYes);
        
  s = (Synth2*)auDestroy((Audio*)s);
  m = (Microphone*)auDestroy((Audio*)m);
  
  if(aiffDurationInSeconds(aiff) != 0)
    self = bode_new(aiff);
...
void bode_init(Bode* self)
{
  ...
  self->is_impedance        = aiffNumChannels(self->aiff) == 2;
...
void bode_calculate_spectrum(Bode* self)
{
  ...
  dft_sample_t* scratch = malloc(self->fft_n * num_channels * sizeof(*self->real));
  if(scratch == NULL) {perror("cannot bode_calculate_spectrum"); return;}
  
  aiffRewindPlayheadToBeginning(self->aiff);
  aiffReadFloatingPointSamplesAtPlayhead (self->aiff, scratch, self->aiff_n * num_channels, aiffYes);
  auDeinterleave(scratch, self->aiff_n, num_channels);
  //if(window != NULL) dft_apply_window (self->real, window, self->fft_n);
  memset(self->real, 0, self->fft_n * sizeof(*self->real) * num_channels);
  memset(self->imag, 0, self->fft_n * sizeof(*self->imag) * num_channels);
  
  for(chan=0; chan<num_channels; chan++)
    {
      memcpy(self->real + chan*self->fft_n, scratch + chan*self->aiff_n, self->aiff_n * sizeof(*scratch));
      dft_real_forward_dft(self->real + chan*self->fft_n, self->imag + chan*self->fft_n, self->fft_n);
      dft_rect_to_polar   (self->real + chan*self->fft_n, self->imag + chan*self->fft_n, self->fft_n);
      dft_magnitude_to_db (self->real + chan*self->fft_n, self->fft_n);
    }

  if(self->is_unit_step)
    for(chan=0; chan<num_channels; chan++)
      for(i=1; i<self->fft_n / 2; i++)
        self->real[i + chan*self->fft_n] -= 20 * log10(1 / (SIN_TWO_PI * dft_frequency_of_bin(i, aiffSampleRate(self->aiff), self->fft_n)));

  if(self->audio_if != NULL)
    for(chan=0; chan<num_channels; chan++)
      for(i=1; i<self->fft_n; i++)
        self->real[i + chan*self->fft_n] -= self->audio_if->real[i];

  //multiple channels?
  if(self->is_face_2_face)
    for(i=1; i<self->fft_n / 2; i++)
       self->real[i] *= 0.5;
  ...
  for(i=max_db; i>=min_db; i -= (max_db - min_db) / 8.0)
    {
      ...
      [[NSString stringWithFormat:@"%d dB", i] drawAtPoint:p withAttributes:fontAttributes];
      p.y += 22;
      p.x = padding;
      [path moveToPoint: p];
      p.x = padding + width;
