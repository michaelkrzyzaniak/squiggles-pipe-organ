...
Harmonizer*       harmonizer_destroy               (Harmonizer* self)
{
  if(self != NULL)
    {
      self->stft            = stft_autocorr_destroy(self->stft);
...
/*-----------------------------------------------------------------------*/
void harmonizer__init_state(Harmonizer* self)
{
  //pthread mutex lock
...
{
  stft_autocorr_process (self->stft, buffer, num_frames, harmonizer_stft_process_callback, self);
}
